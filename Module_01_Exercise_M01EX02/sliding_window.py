...
def slid_win_input():
    ...
    while num_list_value != ".":
        while True:
            num_list_value = input(f"Enter value {num_list_index + 1} in number list (enter . to end list): ")
    ...
    while True:
            k = input(f"Enter k (sliding window): ")
            if k.isnumeric():
                k = int(k)
                break
            else:
...
if __name__ == "__main__":
    sliding_window_function()           # Input: Enter integer number list and k (sliding window)
